...
class BootstrapFilter:
    ...
    def __init__(self, p_x0, transition, likelihood, n, y):
        ...
        Notes
        -----
        This section can be used to provide additional notes. For instance we can LaTeX to write equations related to
        the class.

        .. math:: p(x_t \mid x_{t-1})  \quad p(y_t \mid x_t) \quad p(x_0)

        We can also use it in line. For instance the initial distribution is given by :math:`p(x_0)` the transition
        distribution is :math:`p(x_t \mid x_{t-1})` and the likelihood :math:`p(y_t\mid x_t)`.
    ...
    def run(self):
        """
        Runs the BootstrapFilter algorithm.
